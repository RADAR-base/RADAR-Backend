...
class StreamConfig {
    ...
    @JsonIgnore
    private val priorityThreads: MutableMap<Priority, Int> = EnumMap(Priority::class.java)
    ...
    init {
        priorityThreads[Priority.LOW] = 1
        priorityThreads[Priority.NORMAL] = 2
        priorityThreads[Priority.HIGH] = 4
    }
    ...
    var threadsPerPriority: Map<String, Int>
        ...
        set(streamPriority) {
            ...
            priorityThreads.putAll(streamPriority.entries.associate { (key, value) ->
                Priority.valueOf(key.uppercase(Locale.US)) to value
            })
        }
    ...
    fun threadsByPriority(level: Priority): Int = priorityThreads[level] ?: 1
}
